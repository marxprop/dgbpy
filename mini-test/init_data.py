...
def get_3d_seismic_imgtoimg_data(nrpts=16, nbchunks=1, seed=0, split=0.2, nbfolds=0, nrclasses=5, nr_inattr=1, nr_outattr=1, inpshape = [8,8,8], outshape=[8,8,8]):
    info = get_seismic_imgtoimg_info(nrclasses, nr_inattr, nr_outattr, inpshape, outshape)
    info[dbk.inpshapedictstr] = inpshape
    info[dbk.outshapedictstr] = outshape
    dataset = get_dataset_dict(nrpts)
    info[dbk.datasetdictstr] = dataset
    info = prepare_dataset_dict(info, nbchunks, seed, split, nbfolds)
